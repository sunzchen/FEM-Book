...
# 后处理：计算单元应力及轴力
# ------------------------------------------------------------
def postprocess_elements(coords, IEN, E_list, A_list, d, ndof=2):
    nel = len(IEN)
    results = []
    for e in range(nel):
        n1, n2 = IEN[e]
        ...
        # 如果 E_list 和 A_list 有效则计算应力，否则设为 0 或 None
        if E_list is not None and A_list is not None:
...
def solve_truss(model):
    ...
    E_list = model.get('E')
    A_list = model.get('A')
    elem_results = postprocess_elements(model['coords'], IEN, E_list, A_list, d, ndof)
